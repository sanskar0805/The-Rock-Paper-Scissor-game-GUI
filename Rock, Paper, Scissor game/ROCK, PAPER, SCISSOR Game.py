...
# blocking the resize of window
root.minsize(width=800, height=680)
...
def game(player):
	...
	canvas.create_text(410, 636, text='RESULT:- ' + res,
					fill="black", font=('Algerian', 36), tag='result')
...
rock_b = Button(root, text='Rock', command=lambda: game(1), width=10, bg='#FF69B4')
rock_b.place(x=9, y=502)

# Button for selecting paper
paper_b = Button(root, text='Paper', command=lambda: game(2), width=10, bg='#FEF250')
paper_b.place(x=112, y=502) 

# Button for selecting scissor
scissor_b = Button(root, text='Scissor', command=lambda: game(3), width=10, bg='#87CEEB')
scissor_b.place(x=212, y=502)

# Button for clear
clear_b = Button(root, text='CLEAR', font=('Times', 10, 'bold'),
				width=20, height=2, command=clear, bg="blue").place(x=75, y=550)
